...
function loadCode(code) {
  ...
  debug.run();
  debug.machine.context.evaluate(bootstrapCode);
}
...
function start(max, maxY, steps) {
  ...
  function gotoStep(val) {
    ...
    window.choc.scrub(exampleCode,val,{
      locals: {
                stroke: function(){},
                line: function(){}
              },
      onMessages:function(message){
        if(widget){
          widget.clear();
          widget = null;
        }
        var widgetHtml = dom("<div class='line-messages'>" + message[0].message + "</div>");

        widget = editor.addLineWidget(highlighted, widgetHtml[0]);
    }})
    ...
    if (currentStep === val) {
      return;
    } else if (!currentStep || currentStep > val) {
      debug.once('breakpoint', handler.bind(null, val));
      debug.run();
      debug.machine.context.evaluate(bootstrapCode);
    } else {
      handler(val - currentStep);
    }
    currentStep = val;
  }
